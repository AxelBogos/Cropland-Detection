...
def run_experiment(models=None, do_save_preds=False, use_comet=False, processed_version=None):
    name = os.path.basename(__file__).split('.')[0]
    if processed_version is not None:
        X_train, X_val, y_train, y_val, X_test = load_processed_data(version=processed_version,
                                                                     split_val=True)
    else:
        X_train, X_val, y_train, y_val, X_test = load_orig_data(scaler='standard', split_val=True)

    clf = StackingClassifier(estimators=models, final_estimator=LogisticRegression(), cv=3)
...
def main():
    ...
    agg4_xgb_best_params = {'alpha': 0.006346636300666795,
                            'booster': 'dart',
                            'eta': 0.26489900012668616,
                            'gamma': 1.4664997890899854e-08,
                            'grow_policy': 'depthwise',
                            'lambda': 3.299163146723511e-06,
                            'max_depth': 11,
                            'n_estimators': 3555,
                            'normalize_type': 'forest',
                            'rate_drop': 0.002295458062180691,
                            'sample_type': 'uniform',
                            'scale_pos_weight': 1,
                            'skip_drop': 2.2534613053517485e-06}

    agg4_lgbm_best_params = {'boosting_type': 'gbdt', 'class_weight': None,
                             'colsample_bytree': 1.0, 'importance_type': 'split',
                             'learning_rate': 0.2290373641751674, 'max_depth': 11,
                             'min_child_samples': 20, 'min_child_weight': 0.001,
                             'min_split_gain': 0.0, 'n_estimators': 4600, 'n_jobs': -1,
    ...
    # agg 4
    models.append(('LGBM', lgbm.LGBMClassifier(**agg4_lgbm_best_params)))
    models.append(('XGB', xgb.XGBClassifier(**agg4_xgb_best_params)))
    models.append(('MLP', MLPClassifier()))

    run_experiment(models=models, do_save_preds=True, use_comet=True, processed_version='aggregated_4')
